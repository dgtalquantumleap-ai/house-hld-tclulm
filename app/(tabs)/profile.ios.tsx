...
export default function ProfileScreen() {
  ...
  const handleDeleteAccount = () => {
    console.log('[Profile iOS] Delete account button pressed');
    console.log('[Profile iOS] User authenticated:', !!user);
    console.log('[Profile iOS] Environment:', __DEV__ ? 'Development' : 'Production');
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              setIsDeleting(true);
              console.log('[Profile iOS] Starting account deletion...');
              console.log('[Profile iOS] Environment:', __DEV__ ? 'Development' : 'Production');
              console.log('[Profile iOS] Platform:', Platform.OS);
              
              // Get the current session to get the access token
              const { data: { session }, error: sessionError } = await supabase.auth.getSession();
              
              if (sessionError || !session) {
                console.error('[Profile iOS] No valid session:', sessionError);
                throw new Error('Authentication required. Please sign in again.');
              }

              console.log('[Profile iOS] Session valid, access token present:', !!session.access_token);
              ...
              console.log('[Profile iOS] Calling delete-account Edge Function at:', functionUrl);
              ...
              console.log('[Profile iOS] Response status:', response.status);

              const result = await response.json();

              if (!response.ok) {
                console.error('[Profile iOS] Edge Function error:', result);
                throw new Error(result.error || 'Failed to delete account');
              }

              console.log('[Profile iOS] Account deleted successfully:', result);

              // Sign out and clear all data (this will also clean up Realtime subscriptions)
              console.log('[Profile iOS] Signing out...');
              await signOut();

              // Navigate to login
              console.log('[Profile iOS] Navigating to login...');
              ...
              console.error('[Profile iOS] Account deletion failed:', error);
              Alert.alert(
                "Deletion Failed",
  ...
  console.log('[Profile iOS] Rendering - User:', !!user, 'Environment:', __DEV__ ? 'Dev' : 'Prod');
